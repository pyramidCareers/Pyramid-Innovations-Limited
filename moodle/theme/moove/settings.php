...
if ($ADMIN->fulltree) {
    ...
    /*
    * ----------------------
    * Tablink settings tab
    * ----------------------
    */
    $page = new admin_settingpage('pyramid_Tab_settings', "Pyramid Tab Settings");

    $name ='theme_moove/TabName';
    $title='Tab Name';
    $description='Set the name of the tab';
    $setting=new admin_setting_configtext($name,$title,$description,"Job Portal");
    $page->add($setting);

    $name = 'theme_moove/Tablink';
    $title="Tab Link";
    $description="Set Tab Redirect link";
    $setting = new admin_setting_configtext($name, $title, $description, 'https://www.pyramidinnovation.com/');
    $page->add($setting);
    $settings->add($page);
